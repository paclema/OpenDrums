...
ui.addCheckbox("Flexiglobe connected", 1.75*ui.screenWidth/3 , 70, 500, 100, false).onChange(function(val) { 
    ...
    if(val){
        btClient2=network.bluetooth.connectSerial('98:D3:31:B2:DC:2F', function(status) {
            console.log("btClient2 connected  " + status);
            
            if (status) media.textToSpeech("Pad conectado");  //media.playSound("Sensors_connected.wav");               //media.textToSpeech("Sensores conectados");
    ...
    btClient2.onNewData(function(data) {
        ...
        if(!calibrate_sensors){
            ...
            var volumen = map(sensor_raw[0],0,700,0,100);
            //media.volume(parseInt(volumen));
            media.playSound("mid-tom.wav"); 
